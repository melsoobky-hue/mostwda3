    ...
    <div className="space-y-6">
      ...
      <div className="flex items-center justify-between anim-fade-up">
        <div>
          ...
          <p className="text-xs mt-1.5 leading-relaxed" style={{ color: 'var(--text-muted)' }}>{items.length} items {lowStockCount > 0 && <span style={{ color: 'var(--danger)' }}>· {lowStockCount} low stock</span>}</p>
        </div>
      ...
      <div className="card anim-fade-up stagger-1">
        <div className="flex items-center gap-4">
          <input type="text" placeholder="Search by name or SKU..." value={search} onChange={e => setSearch(e.target.value)} className="input-field flex-1" style={{ background: 'var(--bg-tertiary, var(--bg-secondary))' }} />
            ...
            <table className="w-full">
              ...
              <tbody>
                {items.map(item => {
                  ...
                  return (
                    <tr key={item.id} className="table-row" style={{ transition: 'background 0.15s ease' }}
                      onMouseEnter={e => e.currentTarget.style.background = 'rgba(139,115,85,0.03)'}
                      onMouseLeave={e => e.currentTarget.style.background = ''}>
                      <td className="py-3.5 px-5 text-[11px] font-mono font-semibold" style={{ color: 'var(--accent)' }}>{item.sku}</td>
                      <td className="py-3.5 px-5 text-[11px] font-medium" style={{ color: 'var(--text-primary)' }}>{item.name}</td>
                      <td className="py-3.5 px-5 text-[12px] font-bold" style={{ color: isOut ? 'var(--danger)' : isLow ? 'var(--warning)' : 'var(--text-primary)' }}>{item.stock_quantity}</td>
                      <td className="py-3.5 px-5 text-[11px]" style={{ color: 'var(--text-muted)' }}>{item.low_stock_threshold}</td>
                      <td className="py-3.5 px-5 text-[11px]" style={{ color: 'var(--text-muted)' }}>{item.reorder_point}</td>
                      <td className="py-3.5 px-5 text-[11px] font-medium" style={{ color: 'var(--text-primary)' }}>EGP {(item.cost || 0).toLocaleString()}</td>
                ...
                {items.length === 0 && <tr><td colSpan="8" className="py-14 text-center text-xs" style={{ color: 'var(--text-muted)' }}>No inventory items found</td></tr>}
              </tbody>
            </table>
          </div>
        )}
          ...
          <div className="w-full max-w-md card anim-scale" style={{ boxShadow: '0 8px 40px rgba(0,0,0,0.12)' }} onClick={e => e.stopPropagation()}>
            ...
            <form onSubmit={handleAdd} className="space-y-4">
              <div className="grid grid-cols-2 gap-3.5">
                <div>
                  <label className="text-[10px] font-semibold uppercase tracking-wider mb-1.5 block" style={{ color: 'var(--text-muted)' }}>SKU</label>
                ...
                <div>
                  <label className="text-[10px] font-semibold uppercase tracking-wider mb-1.5 block" style={{ color: 'var(--text-muted)' }}>Name</label>
              ...
              <div className="grid grid-cols-3 gap-3.5">
                <div>
                  <label className="text-[10px] font-semibold uppercase tracking-wider mb-1.5 block" style={{ color: 'var(--text-muted)' }}>Stock</label>
                ...
                <div>
                  <label className="text-[10px] font-semibold uppercase tracking-wider mb-1.5 block" style={{ color: 'var(--text-muted)' }}>Low Threshold</label>
                ...
                <div>
                  <label className="text-[10px] font-semibold uppercase tracking-wider mb-1.5 block" style={{ color: 'var(--text-muted)' }}>Reorder Point</label>
                  <input type="number" value={form.reorder_point} onChange={e => setForm({ ...form, reorder_point: parseInt(e.target.value) || 10 })} className="input-field" style={{ background: 'var(--bg-tertiary, var(--bg-secondary))' }} />
                </div>
              </div>
              <div className="grid grid-cols-2 gap-3.5">
                <div>
                  <label className="text-[10px] font-semibold uppercase tracking-wider mb-1.5 block" style={{ color: 'var(--text-muted)' }}>Cost (EGP)</label>
                ...
                <div>
                  <label className="text-[10px] font-semibold uppercase tracking-wider mb-1.5 block" style={{ color: 'var(--text-muted)' }}>Reorder Qty</label>
                  <input type="number" value={form.reorder_quantity} onChange={e => setForm({ ...form, reorder_quantity: parseInt(e.target.value) || 20 })} className="input-field" style={{ background: 'var(--bg-tertiary, var(--bg-secondary))' }} />
                </div>
              </div>
      ...
      {showRestockModal && (
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4" style={{ background: 'rgba(30,25,20,0.55)', backdropFilter: 'blur(16px)' }} onClick={() => setShowRestockModal(null)}>
          <div className="w-full max-w-sm card anim-scale" style={{ boxShadow: '0 8px 40px rgba(0,0,0,0.12)' }} onClick={e => e.stopPropagation()}>
            ...
            <p className="text-xs mb-3" style={{ color: 'var(--text-muted)' }}>Current stock: <span className="font-bold" style={{ color: 'var(--text-primary)' }}>{showRestockModal.stock_quantity}</span></p>
            <input type="number" value={restockQty} onChange={e => setRestockQty(parseInt(e.target.value) || 0)} className="input-field w-full mb-5" style={{ background: 'var(--bg-tertiary, var(--bg-secondary))' }} placeholder="Quantity to add" />
            <p className="text-xs mb-5" style={{ color: 'var(--text-primary)' }}>New stock: <span className="font-bold" style={{ color: 'var(--success)' }}>{(showRestockModal.stock_quantity || 0) + restockQty}</span></p>
