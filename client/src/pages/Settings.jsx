  ...
  return (
    <div className="space-y-6">
      <div className="anim-fade-up">
        ...
        <p className="text-xs mt-0.5" style={{ color: 'var(--text-muted)' }}>Connection profiles, sync schedule, and system info</p>
      </div>

      {/* Sync Interval */}
      <div className="card anim-fade-up stagger-1" style={{ borderLeft: '3px solid var(--accent)' }}>
        <div className="flex items-center gap-3 mb-4">
          ...
          <div>
            ...
            <p className="text-[11px]" style={{ color: 'var(--text-muted)' }}>Current: <span className="font-semibold" style={{ color: 'var(--accent)' }}>{syncInfo?.nextSync || 'Every 1 hour'}</span></p>
          </div>
        </div>

        <div className="mb-3">
          <p className="text-[10px] font-semibold uppercase tracking-wider mb-2" style={{ color: 'var(--text-muted)' }}>Quick Select</p>
              ...
              <div className="space-y-3">
                ...
                <div className="flex items-center justify-between p-3 rounded-xl" style={{ background: 'var(--bg-secondary)' }}>
                  ...
                  <div className="flex items-center gap-1.5">
                    <div className="w-1.5 h-1.5 rounded-full anim-pulse" style={{ background: statusColor }}></div>
                    <span className="text-xs font-medium" style={{ color: statusColor }}>{hasCreds ? 'Active' : 'No credentials'}</span>
                  </div>
                </div>

                {src.safe && (
                  <div className="p-3 rounded-xl border border-dashed" style={{ borderColor: 'var(--success)', background: 'rgba(16,185,129,0.03)' }}>
                    <p className="text-[10px] font-medium" style={{ color: 'var(--success)' }}>API credentials are stored securely in server config</p>
                  </div>
                )}
                {!src.safe && (
                  <div className="p-3 rounded-xl border border-dashed" style={{ borderColor: 'var(--warning)', background: 'rgba(245,158,11,0.03)' }}>
      ...
      {/* System Info */}
      <div className="card anim-fade-up stagger-6">
        <h3 className="text-sm font-semibold mb-3" style={{ color: 'var(--text-primary)' }}>System Info</h3>
