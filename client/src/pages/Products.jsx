    ...
    <div className="space-y-5">
      <div className="flex items-center justify-between anim-fade-up">
        <div>
          ...
          <p className="text-xs mt-0.5" style={{ color: 'var(--text-muted)' }}>{filtered.length} products {selectedSkus.size > 0 && `· ${selectedSkus.size} selected`}</p>
        </div>
      ...
      {loading ? (
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-4">
          {filtered.map((product, i) => (
            <div key={product.id || i} className="card cursor-pointer group anim-fade-up" style={{ animationDelay: `${Math.min(i * 0.05, 0.4)}s`, padding: 0, overflow: 'hidden' }} onClick={() => setSelectedProduct(product)}>
              <div className="relative h-36 overflow-hidden" style={{ background: 'linear-gradient(135deg, rgba(99,102,241,.1), rgba(6,182,212,.1))' }}>
                {product.image_url ? (
              ...
              <div className="p-4">
                <h3 className="text-xs font-semibold line-clamp-2 mb-1" style={{ color: 'var(--text-primary)' }}>{product.name}</h3>
                <p className="text-[10px] font-mono mb-2" style={{ color: 'var(--text-muted)' }}>{product.sku}</p>
        ...
        <div className="card anim-fade-up stagger-2" style={{ padding: 0, overflow: 'hidden' }}>
          <table className="w-full">
            ...
            <tbody>
              {filtered.map((p, i) => (
                <tr key={p.id || i} className="table-row cursor-pointer" onClick={() => setSelectedProduct(p)}>
                  ...
                  <td className="py-3 px-4 text-xs font-medium" style={{ color: 'var(--text-primary)' }}>{p.name}</td>
                  <td className="py-3 px-4 text-[10px] font-mono" style={{ color: 'var(--text-muted)' }}>{p.sku}</td>
                  <td className="py-3 px-4">{p.mirror_type ? <span className="badge badge-purple">{p.mirror_type}</span> : '-'}</td>
                  <td className="py-3 px-4 text-xs" style={{ color: 'var(--text-secondary)' }}>{p.dimensions || '-'}</td>
                  <td className="py-3 px-4 text-xs font-bold" style={{ color: 'var(--success)' }}>EGP {p.price?.toLocaleString()}</td>
                  <td className="py-3 px-4 text-xs" style={{ color: 'var(--text-secondary)' }}>{p.stock_quantity}</td>
                  <td className="py-3 px-4"><span className={`badge ${p.source === 'mostwda3' ? 'badge-blue' : 'badge-purple'}`}>{p.source}</span></td>
                </tr>
              ))}
            </tbody>
          </table>
      ...
      {selectedProduct && (
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4" style={{ background: 'rgba(0,0,0,.6)', backdropFilter: 'blur(8px)' }} onClick={() => setSelectedProduct(null)}>
          <div className="card w-full max-w-lg anim-scale" onClick={e => e.stopPropagation()}>
            ...
            {selectedProduct.image_url && <img src={selectedProduct.image_url} alt={selectedProduct.name} className="w-full h-48 object-cover rounded-xl mb-4" />}
      ...
      {showBulkUpdate && (
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4" style={{ background: 'rgba(0,0,0,.6)', backdropFilter: 'blur(8px)' }} onClick={() => setShowBulkUpdate(false)}>
          <div className="card w-full max-w-md anim-scale" onClick={e => e.stopPropagation()}>
            ...
            <p className="text-xs mb-4" style={{ color: 'var(--text-muted)' }}>Updating {selectedSkus.size} products</p>
            <div className="space-y-3">
              <div>
                <label className="text-[10px] font-semibold uppercase tracking-wider block mb-1" style={{ color: 'var(--text-muted)' }}>New Price (EGP)</label>
              ...
              <div>
                <label className="text-[10px] font-semibold uppercase tracking-wider block mb-1" style={{ color: 'var(--text-muted)' }}>New Cost (EGP)</label>
