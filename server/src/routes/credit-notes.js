...
import { getCreditNotes, getCreditNoteById, createCreditNote, updateCreditNote, deleteCreditNote } from '../services/database.js';
const router = Router();
...
router.put('/:id', (req, res) => { try { const ok = updateCreditNote(req.params.id, req.body); res.json({ success: ok }); } catch (e) { res.status(500).json({ error: e.message }); } });
